...
# SHARED STATE
# =========================
slot_target = {"x": None, "y": None, "yaw": None}
...
#         lookahead = 1 if sp < 1.5 else 3

#         dist_to_goal = math.hypot(goal_xy[0]-cx, goal_xy[1]-cy)

#         current_lookahead = 1 if dist_to_goal < 4.0 else lookahead

#         ci = min(range(len(path)), key=lambda i: math.hypot(path[i][0]-cx, path[i][1]-cy))
#         ti = min(ci + current_lookahead, len(path)-1)
#         wx, wy = path[ti]
...
def follow_path(vehicle, path, goal_xy):
    while True:
        ...
        # Hard stop when close enough to the approach point
        if dist_to_goal < 0.8:
            vehicle.apply_control(carla.VehicleControl(throttle=0.0, brake=1.0))
            time.sleep(0.3)   # let physics settle before pull_in starts
            break

        ye = normalize_angle_deg(math.degrees(math.atan2(wy-cy, wx-cx)) - yaw)
        st = max(-1.0, min(1.0, ye / 15.0))

        if dist_to_goal < 5.0:
            # Slow approach — crawl and brake if already fast
            if sp > 1.5:
                vehicle.apply_control(carla.VehicleControl(throttle=0.0, brake=0.4, steer=st))
            else:
                th = 0.12 if sp < 1.0 else 0.0
                vehicle.apply_control(carla.VehicleControl(throttle=th, steer=st, brake=0.0))
        else:
...
#     target_yaw = -90.0 if slot_y < LOT_CENTER_Y else 90.0

#     print(f"[PULL_IN] Targeting Slot at ({slot_x:.2f}, {slot_y:.2f}) with Yaw {target_yaw}")
...
#         yaw_error = normalize_angle_deg(target_yaw - yaw)
...
#         if dist_to_actual_slot < 0.3 and abs(yaw_error) < 2.0:
#             print("[PULL_IN] Aligned and in slot!")
#             break

#         if dist_to_actual_slot < 0.15: # Safety stop
#             print("[PULL_IN] Distance limit reached.")
#             break

#         st = max(-1.0, min(1.0, yaw_error / 10.0))
...
# def pull_in(vehicle, slot_x, slot_y, target_yaw):
#     deadline = time.time() + 10.0
#     while time.time() < deadline:
#         t = vehicle.get_transform()
#         cx, cy, yaw = t.location.x, t.location.y, t.rotation.yaw
...
#         # Precision stop prevents "turning past the straight angle"
#         if dist < 0.4 and abs(yaw_err) < 2.0:
#             break

#         st = max(-1.0, min(1.0, yaw_err / 15.0))
#         vehicle.apply_control(carla.VehicleControl(throttle=0.15, steer=st, brake=0.0))
#         time.sleep(0.05)
...
def pull_in(vehicle, slot_x, slot_y, target_yaw):
    """
    Drive straight into the parking slot from the aisle approach point.

    Strategy:
      - Use pure-pursuit steering toward (slot_x, slot_y) — this handles any
        small lateral offset from the approach point naturally.
      - Stop as soon as we are within STOP_DIST of the slot centre, OR the
        10-second deadline expires.  We do NOT require a yaw condition here
        because the car should already be roughly aligned after follow_path.
    """
    STOP_DIST  = 0.25   # metres — how close is "in the slot"
    SLOW_DIST  = 3.0   # metres — start crawling
    THROTTLE_FAST = 0.20
    THROTTLE_SLOW = 0.12
    ...
    while time.time() < deadline:
        ...
        if dist < STOP_DIST:
            break   # we are in the slot

        # Pure-pursuit toward the slot centre
        bearing = math.degrees(math.atan2(slot_y - cy, slot_x - cx))
        ye = normalize_angle_deg(bearing - yaw)
        st = max(-1.0, min(1.0, ye / 15.0))

        th = THROTTLE_SLOW if dist < SLOW_DIST else THROTTLE_FAST
        if sp > 1.8:   # don't overspeed in the slot
            th = 0.0
    ...
    print(f"[PULL_IN] Parked at yaw={final_yaw:.1f}°  (target={target_yaw}°) ✓")
...
def run_demo():
    ...
    sx, sy = slot_target["x"], slot_target["y"]
    syaw = slot_target["yaw"]
    demo_state["phase"] = "running"

    draw_label(world, sx, sy, "SLOT", carla.Color(255, 0, 0))

    # 1. CALCULATE AISLE APPROACH POINT
    # Slots in Town05 run North-South. The aisles run East-West.
    # The car must reach the aisle cell directly in front of the slot mouth,
    # then pull_in drives straight into the slot along the X-axis.
    #
    # syaw =  90 → car will park pointing South (+X in CARLA).
    #              The aisle is on the North side → approach_x = slot_x - dist
    # syaw = -90 → car will park pointing North (-X in CARLA).
    #              The aisle is on the South side → approach_x = slot_x + dist
    approach_dist = 5.0  # metres from slot mouth into the aisle

    if syaw == 90.0:
        sx_adj = sx - approach_dist
    else:   # -90.0
        sx_adj = sx + approach_dist
    sy_adj = sy   # Y stays the same — stay in the same column

    print(f"[DEMO] Slot=({sx:.2f},{sy:.2f}) yaw={syaw}  approach=({sx_adj:.2f},{sy_adj:.2f})")

    # 2. HOLE PUNCH (Safety Net)
    # Clear cells around spawn, approach point, and the actual slot cell.
    for pos in [(ego_x, ego_y), (sx_adj, sy_adj), (sx, sy)]:
        gx, gy = world_to_grid(pos[0], pos[1], origin_x, origin_y)
        for dy in range(-5, 6):
            for dx in range(-5, 6):
...
#     # 1. CALCULATE VERTICAL APPROACH
#     rad = math.radians(syaw)
#     approach_dist = 4.5
#     sx_adj = sx - approach_dist * math.cos(rad)
#     sy_adj = sy - approach_dist * math.sin(rad)

#     # 2. HOLE PUNCH (The Safety Net)
#     # This clears a small area around the start and end so A* never fails.
#     for pos in [(ego_x, ego_y), (sx_adj, sy_adj)]:
#         gx, gy = world_to_grid(pos[0], pos[1], origin_x, origin_y)
#         for dy in range(-4, 5):
#             for dx in range(-4, 5):
#                 if in_bounds(gx+dx, gy+dy):
...
def set_slot():
    ...
    # TOWN05 AISLE LOGIC:
    # Slots run North-South (cars point up or down, yaw = -90 or 90).
    # Rows above the lot centre open southward (car drives in heading South = yaw 90).
    # Rows below the lot centre open northward (car drives in heading North = yaw -90).
    # "Above" in world-Y means larger Y values here.
    slot_target["yaw"] = 90.0 if wy > LOT_CENTER_Y else -90.0

    _demo_trigger.set()
    return jsonify({"message": f"Target locked at ({wx}, {wy}) facing {slot_target['yaw']}°"})
